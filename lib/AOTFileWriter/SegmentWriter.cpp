...
void SegmentsWriter::write_to_buf(char* buf, std::vector<Segment*>& segs, AOT_Header* hdr)
{
      ...
      for(auto seg_ptr : segs)
      {
           ...
           TBWriter::write_to_buf(hdr, cur_seg, tb_table_cur, rel_cur, code_cur, seg_ptr->_tbs);
